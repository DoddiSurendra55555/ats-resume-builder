...
const ResumeForm = () => {
    const { 
        ...
        setGeneratedData, setLoading, loading,
    ...
    if (loading) {
        return (
            <div style={{ maxWidth: '900px', margin: '40px auto', backgroundColor: '#111827', borderRadius: '8px', padding: '30px', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px', borderBottom: '1px solid #374151', paddingBottom: '16px' }}>
                    ...
                    <span style={{ color: '#9ca3af', marginLeft: '8px', fontFamily: 'monospace', fontSize: '14px' }}>Gemini 2.5 AI Agent Active...</span>
                </div>
        ...
        <div className={styles['form-container']}>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className={styles['form-group']}>
                    ...
                    <input type="password" className={styles['form-input']} placeholder="Paste your API key here..." value={apiKey} onChange={(e) => setApiKey(e.target.value)} required />
                </div>
                ...
                <div className={styles['grid-2-col']}>
                    ...
                    <div className={styles['form-group']}><label className={styles['form-label']}>Email Address</label><input type="email" name="email" className={styles['form-input']} value={personalInfo.email} onChange={handleInfoChange} required /></div>
                    <div className={styles['form-group']}><label className={styles['form-label']}>Phone Number</label><input type="tel" name="phone" className={styles['form-input']} value={personalInfo.phone} onChange={handleInfoChange} required /></div>
                    <div className={styles['form-group']}><label className={styles['form-label']}>LinkedIn URL</label><input type="url" name="linkedin" className={styles['form-input']} value={personalInfo.linkedin} onChange={handleInfoChange} /></div>
                </div>

                <div style={{ backgroundColor: theme === 'dark' ? '#451a03' : '#fffbeb', borderLeft: '4px solid #f59e0b', padding: '12px', marginTop: '24px', marginBottom: '16px', borderRadius: '4px', fontSize: '13px', color: theme === 'dark' ? '#fde68a' : '#92400e', transition: 'all 0.3s ease' }}>
                    <strong>💡 Maximize Your ATS Score:</strong> Applicant Tracking Systems require strict timelines and numbers. Make sure to include <strong>Months/Years</strong> for every role and <strong>Metrics</strong> (e.g., "managed 4 people")!
                </div>

                {/* --- EDUCATION SECTION --- */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                    <h3 className={styles['section-title']} style={{ margin: 0, borderBottom: 'none' }}>Education</h3>
                    <button type="button" onClick={() => addArrayItem(setEducation, education, { degree: '', school: '', graduationDate: '' })} style={{ background: '#3b82f6', color: '#ffffff', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>+ Add Education</button>
                </div>
